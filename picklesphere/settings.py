...
# A fallback is provided so CI runners (which have no .env file) can boot
# Django for migrations/tests. Production must set a real SECRET_KEY - the
# guard below refuses to start with an insecure key when DEBUG is off.
SECRET_KEY = config('SECRET_KEY', default='django-insecure-ci-dev-key-not-for-production')

# SECURITY WARNING: don't run with debug turned on in production!
DEBUG = config('DEBUG', default=False, cast=bool)

# Refuse to start in production mode (DEBUG=False) while using the unset fallback.
# CI runners are allowed to use it (GitHub Actions sets CI=true). Note: keys
# generated by `django-admin startproject` also start with 'django-insecure-'
# but are real keys, so the guard matches the exact fallback value only.
if not DEBUG and SECRET_KEY == 'django-insecure-ci-dev-key-not-for-production' and not os.environ.get('CI'):
    raise ImproperlyConfigured(
        'SECRET_KEY is not configured. Set SECRET_KEY in your environment or '
        '.env file (see .env.example). Refusing to run with DEBUG=False and '
        'the insecure default key.'
    )

ALLOWED_HOSTS = config('ALLOWED_HOSTS', default='localhost,127.0.0.1').split(',')
# Add ngrok domains for external tunneling.
# Note: Django matches subdomains with a LEADING DOT ('.ngrok-free.dev'), not
# '*.ngrok-free.dev' — the '*' form is treated as an exact match and never matches.
...
# (leading dot, not '*.' — required by Django's host matching)
CSRF_TRUSTED_ORIGINS += [
